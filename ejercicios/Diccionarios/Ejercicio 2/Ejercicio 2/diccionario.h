	...
	*/
	...
	public:
		...
		Nodo(const tClave& clave, const tValor& valor)
			: _clave(clave), _valor(valor), _iz(NULL), _dr(NULL), _insertado(true) {}
		Nodo(Nodo *iz, const tClave &clave, const tValor &valor, Nodo *dr)
			: _clave(clave), _valor(valor), _iz(iz), _dr(dr), _insertado(true) {}

		Nodo(Nodo* iz, const tClave& clave, const tValor& valor, const tValor& valor2, Nodo* dr)	//añadimos nueva constructora en caso de que se quieran meter 2 valores para una clave
			: _clave(clave), _valor(valor), _valor2(valor2), _iz(iz), _dr(dr), _insertado(true) {}

		tClave _clave;
		bool _insertado;																				//Añadimos campo para que en caso de que el valor haya sido insertado se meta el valor en val2
		tValor _valor;
		tValor _valor2;																				//Añadimos un segundo valor a cada nodo
		Nodo *_iz;
		Nodo *_dr;
	};
	...
	*/
	static Nodo *insertaAux(const tClave &clave, const tValor &valor, Nodo *p) {
		...
		if (p == NULL) {
			...
			if (!p->_insertado) {										//En caso de que la clave no tuviera ningún valor
				p->_valor = valor;
			}	
			else {
				p->_valor2 = valor;										//En caso de que ya se hubiera insertado un valor se añade el valor nuevo a valor2							
				p->_insertado = true;									//Se cambia el valor de insertado a true
			}
			return p;
		}
		else if (clave < p->_clave) {
...
public:
	...
	const tValor& valorPara(const tClave &clave) const { 
		...
		valores[0] = p->_valor;
		valores[1] = p->_valor2;
		return valores;							//Se modifica para que devuelva ambos valores en el orden en el que fueron insertados
	}
