...
public:
	...
	*/
	...
	/*
	Lleva el elemento que está en posición pos de la cola a la primera
	posición de la misma.
	*/
	void llevarAlPrincipio(unsigned int pos) {
		Nodo *prim = _prim, *_ant = NULL;
		if (pos > 0) {
			int i = 0;
			while (i != pos) {
				if (i != pos) {
				...
				prim = prim->_sig;			//Con esto llegamos al elemento
				i++;
			}
			if (prim->_sig != NULL) _ant->_sig = prim->_sig;
			else _ult = _ant;
