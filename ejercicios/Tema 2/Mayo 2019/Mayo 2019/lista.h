...
public:
	...
	void estremece() {
		Nodo* aux = _ult->_ant;
		Nodo* recorre = _ult->_ant;
		while (recorre != NULL) {
			aux = recorre;
			recorre = recorre->_ant;
			if (aux->_elem % 2 != 0) { //es impar, lo ponemos el ultimo
				if (aux == _prim) {
					aux->_sig->_ant = NULL;
					_prim = aux->_sig;
	...
	void intercalar() {
		Nodo* auxIni = _prim;
		Nodo* recorreIni = _prim;
		Nodo* auxFin = _ult;

		bool seguir;

		while (recorreIni->_sig !=  NULL && recorreIni->_sig->_sig != NULL) {
			auxIni = recorreIni;
			
			recorreIni = recorreIni->_sig;
			...
			auxFin->_ant = auxIni;
			auxFin->_sig = auxIni->_sig;

			auxIni->_sig->_ant = auxFin;
			auxIni->_sig = auxFin;
