	...
	*/
	...
	/* COMPLEJIDAD:  
	 INDICA Y JUSTIFICA AQUÍ LA COMPLEJIDAD DE TU IMPLEMENTACIÓN:
		El coste de la función será lineal debido a que los nodos se recorren linealmente una única vez
	 */
    ...
    void invertir() {
		if (_ult != _prim) {
			...
			Nodo* anteUlt = _ult->_ant;
			Nodo* antiPrim = _prim;
			Nodo* sigPrim = _prim->_sig;
			_prim = _ult;
			_prim->_ant = NULL;
			_prim->_sig = antiPrim;
			antiPrim->_ant = _prim;
			...
			Nodo* primero = _prim;
			while (i < _numElems - 2) {
				if (i != 0) primero = primero->_sig;		//Dado que con la primera iteracion hemos variado el primer elemento no nos hace falta volver a cambiarlo en la primera vez
				anteUlt = _ult->_ant;		//2
				sigPrim = primero->_sig;	//1
