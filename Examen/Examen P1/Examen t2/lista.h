	...
	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {
		...
		if (k <= _numElems) {
			...
			Nodo* aux = nullptr;
			int i = 0;
			int nelem_destino = _numElems - k;
			while (i < k) {
				if (i != 0)elemento = elemento->_sig;
				if (i == k - 1) {						//corta el original
					_ult = elemento;
					aux = elemento->_sig;				//cogemos el siguiente elemento
					elemento->_sig = NULL;
					this->_numElems = k;	//Actualizamos la cantidad de elementos que tiene la lista tras cortarla
				}
				i++;
			}
			i = 0;
			destino._numElems = nelem_destino;
			while (i < nelem_destino) {
				if(i != 0)aux = aux->_sig;			//iteramos el valor de aux
				if (i == 0) {
					destino._prim = aux;
					aux->_ant = NULL;					//Como es el primero de la lista habrá que hacer que no tenga valor anterior. Seguia vinculado a la lista anterior

				}
				if (i == nelem_destino - 1) {
					destino._ult = aux;
				}
